...
def fill_fecha(con, fecha, codv, xyv):
    """
...
def write_2_file(codv, values):
    """
    print codv, values to file
    """
    from os.path import join
    fo = open(join(par.DIR_OUT, par.F_OUT), 'w', bufsize=par.BUFSIZE)
    for i in range(len(codv)):
        fo.write('{0}\t{1:.0f}\n'.format(codv[i], values[i]))
    fo.flush()
    fo.close()
